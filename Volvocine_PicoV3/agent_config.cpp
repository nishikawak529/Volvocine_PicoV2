    ...
    if (packetSize) {
      ...
      if (len > 0) {
        ...
        if (parsedBaseFields >= 6) {
            Serial.printf("[INFO] Received parameters: omega=%.2f, kappa=%.2f, center=%.1f, amplitude=%.1f, stop_id=%d, stop_delay=%d, feedback_tau=%.3f, light_tau=%.3f, light_gain=%.2f, light_omega_gain=%.2f, min_amp_ratio=%.2f, prc_n=%d (parsed_prc=%d)\n", omega, kappa, servoCenter, servoAmplitude, stopAgentId, stopDelaySeconds, feedbackTauSec, lightTauSec, lightFeedbackGain, lightFeedbackOmegaGain, minAmplitudeRatio, prcHarmonics, parsedPrcFields);
